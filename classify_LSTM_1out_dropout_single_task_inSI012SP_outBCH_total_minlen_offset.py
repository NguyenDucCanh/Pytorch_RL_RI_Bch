...
for nb_Cell in [30]:
	...
	def find_min(n):
		min_len = 100000000
		for i in range(n):
			filename = '_full_new_clap_new_SI/data_'+str(i+1)+'.txt';
	...
	def load_file(i):
		...
		filename_head = '_full_new_clap_bch_correct/head_'+str(i+1)+'.txt';
	...
	for index_filetest in range(nb_file):
		...
		for nb_ep in range(30):
			...
			savefile_test = 'Data_dropout_singletask_total_inSI012_SP_outBCH/test_predict_classify_LSTM_1out_nbC_'+str(nb_Cell)+'_nbE_'+str(nb_epoch)+'_nbTf_'+str(nb_file_train)+'_iTestF_'+str(index_filetest)+'_'+opt+'.data';
